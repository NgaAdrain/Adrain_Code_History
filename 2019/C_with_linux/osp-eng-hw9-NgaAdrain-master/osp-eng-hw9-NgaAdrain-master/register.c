...
//register from file// //allModify!//
void registerFromFile(){
	FILE * inp;
	Contact buffer;
	inp = fopen("phone.dat","rb");
	if(!inp)
		return;
	else if(fread(&buffer,sizeof(Contact),1,inp)==1)
	{//find file not exist or empty//
		initialization(buffer);
	}
	while(fread(&buffer,sizeof(Contact),1,inp)==1){
		addNode(buffer);
	}
	fclose(inp);
}
...
void writeToFile(){
	Node* temp = head;
	FILE * out;
	out = fopen("phone.dat","wb");	
	while(temp!=NULL){
		fwrite(&(temp->info),sizeof(Contact),1,out);
		temp = temp->next;
	}
	fclose(out);
}
