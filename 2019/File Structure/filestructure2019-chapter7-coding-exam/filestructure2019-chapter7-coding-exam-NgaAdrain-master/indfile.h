...
template <class RecType>//이걸 작동하게 해야함
int TextIndexedFile<RecType>::Append (RecType & record)
{
	char *key = record.Key();//student class에 Key()를 만들어야함!
	int ref = Index.Search(key); //key : SID
...
//생성//
template <class RecType>
FixedFieldBuffer TextIndexedFile<RecType>::GetBuffer() {
	return IndexBuffer;
}
//=============//
template <class RecType>
